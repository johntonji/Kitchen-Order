...
class _AllOrdersState extends State<AllOrders> {
  ...
  String orderSelectedItem = 'ALL ORDERS';
  String orderSelectedFilter = 'LAST 7 DAYS';
  ...
  void getData() {
    SharedPreferenceManager.getInstance().getUserData().then((value) {
      userModel = value;
      Provider.of<AppProvider>(context, listen: false).allOrders(orderSelectedItem, orderSelectedFilter);
    ...
    return Consumer<AppProvider>(builder: (context, provider, isChild) {
      ...
      return Column(
        children: [
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                DropdownMenu(
                  width: 150,
                  initialSelection: orderSelectedItem,
                  trailingIcon: Icon(MdiIcons.chevronDown, color: AppAssets.primaryColor),
                  selectedTrailingIcon: Icon(MdiIcons.chevronUp, color: AppAssets.primaryColor),
                  textStyle: TextStyle(fontSize: 12, fontFamily: AppAssets.nunitoBold, color: AppAssets.primaryColor),
                  ...
                  menuStyle: MenuStyle(
                    ...
                    backgroundColor: MaterialStateProperty.all(AppAssets.whiteColor),
                    shadowColor: MaterialStateProperty.all(AppAssets.textLightGrayColor.withOpacity(0.2)),
                    surfaceTintColor: MaterialStateProperty.all(AppAssets.whiteColor),
                    side: MaterialStateBorderSide.resolveWith((states) => const BorderSide(color: AppAssets.primaryColor))
                  ),
                  onSelected: (newValue) {
                    setState(() {
                      orderSelectedItem = newValue!;
                      getData();
                    });
                  },
                  dropdownMenuEntries: orderItems.map<DropdownMenuEntry<String>>(
                          (item) {
                        return DropdownMenuEntry<String>(
                          ...
                          label: item,
                          style: ButtonStyle(
                            textStyle: MaterialStateTextStyle.resolveWith((states) => TextStyle(fontSize: 12, fontFamily: AppAssets.nunitoBold, color: AppAssets.primaryColor),)
                          )
                        );
                      }).toList(),
                ),
                DropdownMenu(
                  width: 150,
                  initialSelection: orderSelectedFilter,
                  trailingIcon: Icon(MdiIcons.chevronDown, color: AppAssets.primaryColor),
                  selectedTrailingIcon: Icon(MdiIcons.chevronUp, color: AppAssets.primaryColor),
                  textStyle: TextStyle(fontSize: 12, fontFamily: AppAssets.nunitoBold, color: AppAssets.primaryColor),
                  inputDecorationTheme: const InputDecorationTheme(
                  ...
                  onSelected: (newValue) {
                    setState(() {
                      orderSelectedFilter = newValue!;
                      getData();
                    });
                  },
                  dropdownMenuEntries: filterItems.map<DropdownMenuEntry<String>>(
                          (item) {
                        return DropdownMenuEntry<String>(
                            value: item,
                            enabled: true,
                  ...
                  elevation: 2,
                  iconSize: 20,
                  icon: Icon(MdiIcons.chevronDown, color: AppAssets.primaryColor),
                  items: orderItems.map((String items) {
                    return DropdownMenuItem(
                      value: items,
                      child: Text(items, style: TextStyle(fontSize: 12, fontFamily: AppAssets.nunitoBold, color: AppAssets.primaryColor), maxLines: 1, overflow: TextOverflow.ellipsis,),
                    );
                  }).toList(),
          ...
          provider.allOrdersList.isNotEmpty ?
          Expanded(
            child: ListView.builder(
              physics: const BouncingScrollPhysics(),
              itemCount: provider.allOrdersList.length,
              itemBuilder: (context, index) {
                final order = provider.allOrdersList[index];
                print(Provider.of<AppProvider>(context, listen: false).daysBetween(DateTime.parse(order.orderData!.dateCreated!), DateTime.now()));
                return Container(
                            ...
                            child: Row(
                              children: [
                                CircleAvatar(
                                  ...
                                  radius: 22,
                                  child: SvgPicture.asset(order.orderData!.status == "new" ? AppAssets.bagIcon : order.orderData!.status == "accepted" ? AppAssets.preparingIcon : AppAssets.bagIcon, colorFilter: ColorFilter.mode(AppAssets.tabBorderColor, BlendMode.srcIn), height: 20, width: 20,),
                                ),
                                  ...
                                  children: [
                                    ...
                                    Row(
                                      children: [
                                        ...
                                        Text(order.orderData!.status == "new" ? "pending..." : order.orderData!.status == "accepted" ? "Preparing..." : "Delivering...", style: TextStyle(fontFamily: AppAssets.nunitoRegular, fontSize: AppAssets.dimen_12, color: AppAssets.widgetGrayColor), maxLines: 1, overflow: TextOverflow.ellipsis,),
                                      ],
                                    ),
                                  ],
                                )),
